    ...
    <nav class="navbar navbar-expand-lg bg-light">
        ...
        <?php
            ...
            $sql = "SELECT * FROM service_record
                    ...
                    AND (serviceType LIKE '%".$searchkeyword."%' 
                    OR serviceDescription LIKE '%".$searchkeyword."%' 
                    OR serviceDuration LIKE '%".$searchkeyword."%' 
                    OR servicePrice LIKE '%".$searchkeyword."%' 
                    OR petType LIKE '%".$searchkeyword."%'
                    OR serviceAverageRating LIKE '%".$searchkeyword."%')";
          ...
          <div class="tableDesign">
              <table class="level1-table">
                ...
                echo    '<tr>
                      ...
                      <td class="viewdoc"> <img height src="images/'.$row['serviceImage'].'" alt=""></td> 
                      <td>'.$row['serviceDuration'].'</td>
                      <td>'.$row['servicePrice'].'</td>
                      <td>'.$row['petType'].'</td>
                      <td>'.$row['serviceAverageRating'].'</td>
                      <td>';
                      ...
                      echo '</td>
                        ...
                        if($row['service_status']=='Active'){
                            echo '<form action="deactivate_service.php" method="post">
                                    <input type="hidden" name="serviceID" value="'.$row['serviceID'].'">
                                    <input type="hidden" name="mainServiceID" value="'.$row['mainServiceID'].'">
                                    <button type="submit" class="red-button"  onclick="return confirm(\'Deactivate '.$row['serviceType'].'?\')">Deactivate</button>
                                </form>';
                        }else{
                            echo '<form action="activate_service.php" method="post">
                                    <input type="hidden" name="serviceID" value="'.$row['serviceID'].'">
                                    <input type="hidden" name="mainServiceID" value="'.$row['mainServiceID'].'">
                                    <button type="submit" class="green-button" onclick="return confirm(\'Activate '.$row['serviceType'].'?\')">Activate</button>
                                 </form>';
                        }
                      
                    echo '</td>

                    <td class="links"><a href="edit_service_type.php?serviceID='.$row['serviceID'].'">Edit</a></td>
